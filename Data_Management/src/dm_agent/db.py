...
-- Per-modality statistics within a dataset (subject counts + lists)
CREATE TABLE IF NOT EXISTS catalog_modality_stats (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    dataset_id INTEGER REFERENCES catalog_datasets(id),
    ...
    subject_list TEXT,          -- JSON list of subject IDs
    dir_count INTEGER DEFAULT 0,
    data_stages TEXT,           -- JSON list of data stages
    key_dirs TEXT,              -- JSON list of key directories
    description TEXT,           -- Claude-generated description
    notes TEXT,                 -- Claude-generated notes
    UNIQUE(dataset_id, modality)
);
...
class Database:
    ...
    # --- catalog_modality_stats ---

    def upsert_modality_stats(
        self,
        dataset_id: int,
        modality: str,
        file_count: int,
        ...
        with self._connect() as conn:
            row = conn.execute(
                "SELECT id FROM catalog_modality_stats WHERE dataset_id = ? AND modality = ?",
                (dataset_id, modality),
            ).fetchone()
            if row:
                conn.execute(
                    """UPDATE catalog_modality_stats SET
                       file_count = ?, size_bytes = ?, subject_count = ?,
                       subject_list = ?, dir_count = ?, data_stages = ?,
                       key_dirs = ?, description = ?, notes = ?
                       WHERE id = ?""",
                    (file_count, size_bytes, subject_count, subject_list,
                     dir_count, data_stages, key_dirs, description, notes, row["id"]),
                )
                return row["id"]
            else:
                cursor = conn.execute(
                    """INSERT INTO catalog_modality_stats
                       (dataset_id, modality, file_count, size_bytes, subject_count,
                        subject_list, dir_count, data_stages, key_dirs, description, notes)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                    (dataset_id, modality, file_count, size_bytes, subject_count,
                     subject_list, dir_count, data_stages, key_dirs, description, notes),
                )
    ...
    def get_modality_stats(self, dataset_id: int) -> List[Dict[str, Any]]:
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT * FROM catalog_modality_stats WHERE dataset_id = ? ORDER BY modality",
                (dataset_id,),
            ).fetchall()
            return [dict(r) for r in rows]

    def delete_modality_stats(self, dataset_id: int) -> None:
        with self._connect() as conn:
            conn.execute(
                "DELETE FROM catalog_modality_stats WHERE dataset_id = ?",
                (dataset_id,),
            )
